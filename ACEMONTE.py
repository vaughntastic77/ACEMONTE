...
Author: Jacob Vaughn
Date: 20 July, 2024
Location: National Aerothermochemistry and Hypersonics Laboratory, Texas A&M University
...
Load_source = '../data/inputdata_5993.pkl'
...
"""
U_systematic.add_error_source(measurements=['P01'],distribution='norm',params=[0,2.5],source='b_P01',units='psi') # 200*0.025/2
U_systematic.add_error_source(measurements=['T0'],distribution='norm',params=[0,2.65],source='b_T0',units='K') # 530*0.01/2
U_systematic.add_error_source(measurements=['P'],distribution='norm',params=[0,15],source='b_P',units='Pa') # 600*0.05/2
U_systematic.add_error_source(measurements=['P02'],distribution='norm',params=[0,2.6],source='b_P02',units='Torr') # 520*0.01/2
...
if s_flag == 'P':
    ...
    U_random.add_error_source(measurements=['P01'],distribution='norm',params=[0,1],source='s_P01',units='psi')
    U_random.add_error_source(measurements=['T0'],distribution='norm',params=[0,1],source='s_T0',units='K')
    U_random.add_error_source(measurements=['P'],distribution='norm',params=[0,5],source='s_P',units='Pa')
    U_random.add_error_source(measurements=['P02'],distribution='norm',params=[0,1],source='s_P02',units='Torr')
...
print('Run ' + str(testinfo['Run Number']) + ' complete')
...
manual_plots = False # Plotting method:
#           True for hard coded plotting lines below
#           False for WINDMONTE_GUI interactive plotting tool
Plot_Xvar = 'P01'
Plot_VOIs = ['Re(P)','M(P)','P','Re(P02)','M(P02)','P(P02)']

if manual_plots:
    # Plot directly using plot behaviors for the TestRun, DataPoint, or VOI variable class
    RunData.plot_errorbars(Plot_Xvar,Plot_VOIs)
    RunData.plot_U_VOI(Plot_Xvar,Plot_VOIs)
    RunData.plot_U_and_UPCs(Plot_Xvar,Plot_VOIs)
    plt.show()
else:
    # Running GUI is the default
    ACEMONTE_GUI.launch(Plot_Xvar,','.join(Plot_VOIs))
    #os.system("python3 WINDMONTE_GUI.py")
#endregion
